...
import com.kele.core.other.util.minio.MinioUtils;
...
@Slf4j
@Service
@ConditionalOnProperty(prefix = "kele.doc.docStorage", name = "type", havingValue = "minio")
public class MinioDocFileContentStorageServiceImpl extends AbstractFileSystemStorageService {
    ...
    public MinioDocFileContentStorageServiceImpl(KeleDocWebProperties lxDocWebProperties) {
    ...
    public boolean create(DocFileFolder fileFolder) {
        String objectName =
            this.getFilePath(fileFolder, fileFolder.getVersion());
        this.upload(() -> {
    ...
    public String computeFileHash(DocFileFolder docFileFolder) {
        String filePath = this.getFilePath(docFileFolder, docFileFolder.getVersion());
    ...
    public boolean delete(DocFileFolder docFileFolder) {
        String objectName = this.getFilePath(docFileFolder, docFileFolder.getVersion());
        try {
