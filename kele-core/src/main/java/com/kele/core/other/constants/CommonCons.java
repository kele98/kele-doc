package com.kele.core.other.constants;

/**
 * @author wuzhenhong
 * @date 2024/5/14 9:11
 */
public class CommonCons {

    public static final String TOKEN_KEY = "lxDocToken";

    // 如果有条件最好不要硬编码，有安全问题，可以使用配置中心或存数据库都行
    public static final String AES_KEY = "23423jAFsdf323ls";

    public static final Integer LOGIN_EXPIRE_SECONDS = 24 * 60 * 60;
    public static final Integer LOGIN_TOKEN_EXPIRE_SECONDS = 36 * 60 * 60;

    public static final String LOGIN_USER_TOKE_KEY = "LOGIN_USER_TOKE_KEY:USER_ID:";

    public static final String FORWARD_SLANT = "/";

    public static final String FS_URL_PREFIX = "/fs/";
}
